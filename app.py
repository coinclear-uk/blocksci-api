...
def address_transactions_from(address):
	return json_success(address_transactions_pagnated(True, address,1))
...
def address_transactions_pagnated(is_from, address_string, page):
    ...
    for transaction in transactions_as_list[start:end]:
        ...
        for tx_input in transaction.inputs:
            input_data = {'address':tx_input.address.address_string,'value':tx_input.value}
            t['inputs'].append(input_data)
        for tx_output in transaction.outputs:
            output_data = {'address':tx_output.address.address_string,'value':tx_output.value,'spent':tx_output.is_spent}
